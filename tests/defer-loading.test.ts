 ...
 *   - Deferred tools' docs do NOT count toward info.toolDocsChars, and are not
 *     rendered into the imaged Tool Reference (info.imageSourceText).
...
const DEFERRED_DESC = 'Query the observability backend for traces. '.repeat(40);
...
describe('defer_loading tools pass through the tool rewrite', () => {
  it('leaves deferred tools byte-identical and still rewrites the rest', async () => {
    const { body, info } = await transformRequest(makeReq([searchTool, deferredTool, plainTool]), {});
    ...
    expect(outDeferred.input_schema.properties.query.description).toBe('Trace search query');

    // Plain tool: still rewritten — stub description citing the Tool Reference,
    // schema annotations stripped.
    ...
    expect(info.toolDocsChars!).toBeLessThan(PLAIN_DESC.length + DEFERRED_DESC.length);
  });

  it('keeps deferred docs out of the imaged Tool Reference', async () => {
    // Moderate slab: clears the compression gate while keeping the Tool
    // Reference inside the imageSourceText capture window (the imaged text is
    // system slab first, then the reference — an 80k slab pushes it past the
    // window and the assertion would miss a heading that IS imaged).
    const req = enc.encode(
      JSON.stringify({
        model: 'claude-3-5-sonnet',
        system: 'x'.repeat(8_000),
        tools: [deferredTool, plainTool],
        messages: [{ role: 'user', content: 'hello' }],
      }),
    );
    const { info } = await transformRequest(req, {});
    // imageSourceText is the exact text rendered into the slab image(s): the
    // plain tool's heading is there, the deferred tool's is not.
    const imaged = info.imageSourceText ?? '';
    expect(imaged).toContain('## Tool: bash');
    expect(imaged).not.toContain('## Tool: mcp__datadog__get_traces');
    // The deferred tool's verbose description was never imaged.
    expect(imaged).not.toContain(DEFERRED_DESC);
  });
  ...
  it('stays byte-identical when every tool is deferred and the request falls below the compression gate', async () => {
    ...
    // the request lands under the compression gate, taking the passthrough
    // return. The passthrough this suite verifies must still hold on that path.
    const req = {
      model: 'claude-3-5-sonnet',
      system: 'hi',

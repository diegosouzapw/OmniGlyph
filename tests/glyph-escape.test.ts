 ...
 *     1-bit (production DENSE_RENDER_STYLE) and the AA grayscale paths
...
describe('rendering emoji', () => {
  ...
  it('production dense style (DENSE_RENDER_STYLE): droppedChars === 0', async () => {
  ...
  it('AA grayscale path (style.aa): droppedChars === 0', async () => {
    // Escape decisions key off the 1-bit atlasRank; the escape spells only
    // ASCII, guaranteed present in BOTH atlases, so the AA path drops nothing.
    const img = await renderChunkToPng(
      'metrics \u{1F4CA} look good \u{1F389}',
      80,
      { aa: true },

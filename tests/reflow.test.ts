...
describe('reflow L0 contract – real corpus', () => {
  ...
  it('L0 contract holds for ~/.omniglyph/4xx-bodies/ if present', () => {
    const dir4xx = resolve(homedir(), '.omniglyph', '4xx-bodies');
    if (!existsSync(dir4xx)) {
      console.log('[4xx-bodies] ~/.omniglyph/4xx-bodies not found — skipping');
      return;
    }

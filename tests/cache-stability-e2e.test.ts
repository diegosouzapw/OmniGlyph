 ...
 *   our input = OmniGlyph's transform of the request body
 *
...
describe('e2e cache alignment — Anthropic /v1/messages through the real proxy', () => {
  ...
  it('ENV SPLIT: a git-status change in `# Environment` never re-renders the slab image', async () => {
    ...
    expect(lastUserText(cap2.main[0]!.body)).toMatch(
      /<system-reminder>[\s\S]*relocated by OmniGlyph[\s\S]*# Environment[\s\S]*<\/system-reminder>/,
    );
  });
  ...
  it('GATE: an out-of-scope model is forwarded byte-for-byte untouched (no images)', async () => {
    // claude-3-5-sonnet is NOT in the default OMNIGLYPH_MODELS scope → passthrough.
...
describe('e2e cache alignment — GPT (OpenAI) through the real proxy', () => {
  ...
  it('responses APPEND-ONLY: completed-pair pages are byte-identical as native state grows', async () => {
    ...
    const body = (pairs: Array<Record<string, unknown>>) => JSON.stringify({
      model: 'gpt-5.6', instructions: slab(60_000),
    ...
    expect(b[0]).toBe(a[0]);
    const parsed = JSON.parse(cap2.main[0]!.body) as { input: Array<Record<string, unknown>> };
    const calls = new Set(parsed.input.filter((x) => x.type === 'function_call').map((x) => x.call_id));
    const outputs = parsed.input.filter((x) => x.type === 'function_call_output');
    expect(outputs.every((x) => calls.has(x.call_id))).toBe(true);
  });

...
//
// Goal (the whole point of routing `omniglyph export` through the SDK): a given text must
// render to the SAME PNG bytes whether it goes through the `omniglyph export` CLI or the
...
// export CLI's default and the proxy's hardcoded dense cap. `omniglyph export --cols N` with

 ...
 * (savings-honesty / savings-math), this asserts the three things OmniGlyph is
 ...
 *   our input = OmniGlyph's transform of the request
 *
...
describe('design: RECENT REQUEST stays legible (GPT)', () => {
  it('images the system slab but keeps the most-recent user request as readable text', async () => {
    ...
    // 12 turns × 2000 chars render ~5s on an unloaded machine — parallel CI
    // workers push it past vitest's 5s default, so give this e2e real headroom.
  }, 30_000);
});

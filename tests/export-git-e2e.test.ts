...
// End-to-end coverage of the real `omniglyph export --git` path (the bug PR-4 fixed
...
describe('omniglyph export --git (end-to-end)', () => {
  let repo: string;
  let outDir: string;
  beforeEach(() => {
    repo = fs.mkdtempSync(path.join(os.tmpdir(), 'OmniGlyph-git-e2e-'));
    outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'OmniGlyph-git-out-'));
    git(repo, ['init', '-q']);

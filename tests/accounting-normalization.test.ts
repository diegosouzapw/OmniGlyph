...
describe('provider input normalization', () => {
  it('sums Anthropic disjoint input and cache buckets', () => {
  ...
  it.each(['openai', 'xai'] as const)(
    'does not double-count the cache subset for %s usage',
    (provider) => {
      expect(providerActualInputTokens({
        provider,
  ...
  it.each([-1, Number.NaN, 1.5, Number.MAX_SAFE_INTEGER + 1])(
    'fails closed on malformed Anthropic cache-read counter %s',
    (cacheReadTokens) => {
      expect(providerActualInputTokens({
        provider: 'anthropic',
        providerInputTokens: 1_000,
...
describe('normalized accounting evidence', () => {
  it('prefers provider-reported reduction over estimates', () => {
  ...
  it('labels savings as estimated when no provider baseline exists', () => {
    const result = normalizeAccounting({
      provider: 'openai',
  ...
  it('falls back to estimates when provider evidence is malformed', () => {
    const result = normalizeAccounting({
      provider: 'anthropic',
    ...
    expect(result.savings.evidence).toBe('estimated');
    expect(result.savings.inputTokensReduced).toBe(6_000);
  });
  ...
  it('reports bytes-only evidence without inventing token savings', () => {
    ...
    expect(result.savings).toEqual({ evidence: 'bytes-only' });
    expect(result.bytes.reduced).toBe(600);
  ...
  it('allows fractional latency while rejecting negative latency', () => {
    const result = normalizeAccounting({
      provider: 'openai',
      proxyAddedLatencyMs: 12.75,
      modelLatencyMs: -1,
    });
    expect(result.latency).toEqual({ proxyAddedMs: 12.75 });
  });

  it('preserves measured expansion as a negative reduction', () => {
    const result = normalizeAccounting({
      provider: 'openai',

 ...
 *
 * Cache tests (cache-stability-e2e) prove OmniGlyph doesn't bust the cache. THIS
 * file proves the prior question: OmniGlyph's gate gets the MATH right, so it never
 ...
 * running OmniGlyph at all), with no error.
 *
 *   fake api  = the upstream output (canned response + count_tokens ground truth)
 *   our input = OmniGlyph's transform + gate decision, read off the onRequest event
 *

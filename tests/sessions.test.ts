...
function makeTmp(): SessionsPaths {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'OmniGlyph-sessions-'));
...
describe('aggregateSessions', () => {
  ...
  it('credits the real prefix compression (image prefix fewer tokens than text prefix)', async () => {
    writeEvents(tmp, [
      // First TRACKED turn, but cr=100 > 0 ⇒ the cache was OBSERVABLY warm (OmniGlyph
  ...
  it('reports a real NEGATIVE when cache_create overhead exceeds the prefix saving; probe-miss credits 0', async () => {
    writeEvents(tmp, [
      ...
      // Genuine loss turn: tiny body (2000) but OmniGlyph wrote 5000 cache_create.
...
describe('filterSessions', () => {
  it('filters by project (substring match)', async () => {
    writeEvents(tmp, [
      ev({ first_user_sha8: 'aaaaaaaa', cwd: '/Users/me/code/omniglyph' }),
      ev({ first_user_sha8: 'bbbbbbbb', cwd: '/Users/me/code/other' }),
    ]);
    const { sessions } = await aggregateSessions(tmp);
    expect(filterSessions(sessions, { project: 'omniglyph' }).map((s) => s.id)).toEqual([
      'aaaaaaaa',
    ]);
...
describe('Claude Code session map', () => {
  ...
  function makeCCRoot(): string {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'OmniGlyph-ccmap-'));
  }
  ...
  it('fingerprints the first user message and maps to the session id', async () => {
    ...
    const proj = path.join(root, '-Users-me-code-omniglyph');
    ...
    expect(ref!.projectPath).toBe('/Users/me/code/omniglyph');

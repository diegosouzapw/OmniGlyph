...
// Tests below assert DEFAULT model-scope behavior, which assumes OMNIGLYPH_MODELS is unset.
// Snapshot and clear any ambient value (e.g. a dev shell that still exports OMNIGLYPH_MODELS)
...
let ambientOmniGlyphModels: string | undefined;
beforeEach(() => {
  ambientOmniGlyphModels = process.env.OMNIGLYPH_MODELS;
  delete process.env.OMNIGLYPH_MODELS;
});
afterEach(() => {
  if (ambientOmniGlyphModels === undefined) delete process.env.OMNIGLYPH_MODELS;
  else process.env.OMNIGLYPH_MODELS = ambientOmniGlyphModels;
});
...
describe('public library API', () => {
  it('recognizes Fable 5 (with suffix aliases) as the default scope; Opus is OFF by default', () => {
    ...
    // it via OMNIGLYPH_MODELS or the dashboard "compress models" chips.
    expect(isOmniGlyphSupportedModel('claude-opus-4-8')).toBe(false);
    // older Opus + other families are not in the default scope
    expect(isOmniGlyphSupportedModel('claude-opus-4-7')).toBe(false);
    expect(isOmniGlyphSupportedModel('claude-opus-4-6')).toBe(false);
    expect(isOmniGlyphSupportedModel('claude-mythos-5')).toBe(false);
    expect(isOmniGlyphSupportedModel('claude-fable-50')).toBe(false);
    expect(isOmniGlyphSupportedModel('claude-sonnet-4-7')).toBe(false);
    expect(isOmniGlyphSupportedModel(null)).toBe(false);
  });

  it('strips bracketed variant tags like [1m] before matching', () => {
    expect(isOmniGlyphSupportedModel('claude-fable-5[1m]')).toBe(true);
    expect(isOmniGlyphSupportedModel('claude-fable-5-high[1m]')).toBe(true);
    expect(isOmniGlyphSupportedModel('claude-opus-4-8[1m]')).toBe(false); // Opus opt-in, off by default
    // a non-scoped base is still rejected even with a variant tag
    expect(isOmniGlyphSupportedModel('claude-opus-4-7[1m]')).toBe(false);
  });

  it('honors OMNIGLYPH_MODELS to override the default scope', () => {
    const prev = process.env.OMNIGLYPH_MODELS;
    try {
      // narrow to Fable only
      process.env.OMNIGLYPH_MODELS = 'claude-fable-5';
      expect(isOmniGlyphSupportedModel('claude-fable-5')).toBe(true);
      expect(isOmniGlyphSupportedModel('claude-opus-4-8')).toBe(false);
      // re-point to a different set
      process.env.OMNIGLYPH_MODELS = 'claude-fable-5,claude-opus-4-7';
      expect(isOmniGlyphSupportedModel('claude-opus-4-7')).toBe(true);
      expect(isOmniGlyphSupportedModel('claude-opus-4-8')).toBe(false); // not in this set
    } finally {
  ...
  it('honors the dashboard runtime override (setAllowedModelBases) over env/default', () => {
    try {
      ...
      expect(isOmniGlyphSupportedModel('claude-opus-4-8')).toBe(true); // opted in at runtime
  ...
  it('recognizes GPT 5.6 as the default OpenAI imaging scope (5.5 opt-in)', () => {
    expect(isOmniGlyphSupportedGptModel('gpt-5')).toBe(false);
    // gpt-5.5 degrades on imaged context, so it is off by default now.
    expect(isOmniGlyphSupportedGptModel('gpt-5.5')).toBe(false);
    expect(isOmniGlyphSupportedGptModel('gpt-5.5-codex')).toBe(false);
    expect(isOmniGlyphSupportedGptModel('gpt-5.5-2026-06-01')).toBe(false);
    expect(isOmniGlyphSupportedGptModel('gpt-5.6')).toBe(true);
    expect(isOmniGlyphSupportedGptModel('gpt-5-mini')).toBe(false);
    expect(isOmniGlyphSupportedGptModel('gpt-5.6-nano')).toBe(true);
    expect(isOmniGlyphSupportedGptModel('gpt-5.6[1m]')).toBe(true);
    expect(isOmniGlyphSupportedGptModel('gpt-4o')).toBe(false);
    expect(isOmniGlyphSupportedGptModel('gpt-50')).toBe(false);
  ...
  it('honors the single OMNIGLYPH_MODELS scope for GPT families', () => {
    ...
    try {
      ...
      process.env.OMNIGLYPH_MODELS = 'claude-fable-5';
      expect(isOmniGlyphSupportedGptModel('gpt-5.5')).toBe(false);
      expect(isOmniGlyphSupportedGptModel('gpt-5.6')).toBe(false);
      ...
      process.env.OMNIGLYPH_MODELS = 'off';
      expect(isOmniGlyphSupportedGptModel('gpt-5.6')).toBe(false);
      expect(isOmniGlyphSupportedModel('claude-fable-5')).toBe(false);
    } finally {
      if (prev === undefined) delete process.env.OMNIGLYPH_MODELS;
  ...
  it('wraps the transformer with model gating and cache ownership metadata', async () => {
    ...
    // Task #21: OmniGlyph never adds its own cache_control markers.

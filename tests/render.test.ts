...
describe('transform', () => {
  ...
  it('moves tool docs into the imaged Tool Reference and stubs originals', async () => {
    ...
    // system prompt"): OmniGlyph-authored framing must never read as a replayed or
  ...
  it('ships annotation-stripped schemas in tools[], full schema in the imaged reference', async () => {
    ...
    // $schema SURVIVES (upstream #45): without it the API validates as
    // 2020-12 and rejects draft-07 constructs (tuple-form items) with a 400 in a loop.
  ...
  it('never adds its own cache_control marker (Task #21)', async () => {
    // Per Task #21: OmniGlyph must NEVER add cache_control markers of its
  ...
  it('extracts env fields (cwd, platform, today, isGitRepo, branch) into info.env', async () => {
    const sys =
      'claude.md\n'.repeat(400) +
      "<env>\n" +
      'Working directory: /Users/me/code/OmniGlyph\n' +
      'Is directory a git repo: Yes\n' +
      'Platform: darwin\n' +
    ...
    expect(info.env!.cwd).toBe('/Users/me/code/OmniGlyph');
  ...
  it('adds no cache_control of its own (Task #21: honor caller markers only)', async () => {
    // OmniGlyph must never add cache_control markers. The caller's slot
...
// Spleen 5×8 glyph-confusability guard. The stock font's 'K' is 'H' with a
// single crossbar pixel removed (Hamming 1) — indistinguishable once the page
// downscales, so imaged 'K' reads back as 'H'. gen-atlas.ts surgeries 'K' to a
// diagonal-legged bitmap; this guard fails closed if that regresses. Letters
// and digits only — the recall-critical classes (identifiers, hex). Punctuation
// pairs that sit at d=1 by design (',;' '.:' — same 5×8 box) are out of scope.

#!/usr/bin/env bash
# Restart the local OmniGlyph proxy.
#
# What this does, in order:
#   1. Discover every running OmniGlyph proxy via `pgrep -f "node.*bin/cli.js"`
...
# --- Parse our own flags out of "$@". --no-build only — OmniGlyph takes none. ----
DO_BUILD=1
for arg in "$@"; do
  case "$arg" in
    --no-build)
    ...
    *)
      ...
      echo "[restart] this script only accepts --no-build (OmniGlyph takes no flags)" >&2
      exit 2
      ;;
  esac
done
...
if [ -n "$PIDS_RAW" ]; then
  ...
  echo "[restart] found running OmniGlyph proxy PID(s): $PIDS"
...
if command -v lsof >/dev/null 2>&1; then
  ...
  if [ -n "$HOLDER" ]; then
    ...
    echo "    $HOLDER_CMD" >&2
    echo "  Hint: if that's a OmniGlyph proxy our SIGTERM should have cleared," >&2

...
// CLI message catalogs are read from disk at runtime (only en.json is inlined
// in the bundle); tsc does not copy JSON, so ship them alongside the bundle.
await cp('src/i18n/messages', 'dist/i18n/messages', { recursive: true });
console.log('✓ copied i18n message catalogs');
...
await build({
  ...
  format: 'esm',
  sourcemap: true,
  // Inline the package version so `OmniGlyph --version` is correct for global/npx
  // installs (see the note where `pkg` is read). esbuild replaces the bare
  // identifier with the string literal at every reference.
  define: { __OMNIGLYPH_VERSION__: JSON.stringify(pkg.version) },

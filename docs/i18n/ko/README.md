...
**세션 도중에 켰더니 사용량이 급증했어요 — 왜죠?**
OmniGlyph 없이 진행되던 세션은 프리픽스 전체가 Anthropic에 텍스트로 0.1× 읽기 요율로 캐시되어 있습니다. 이미지가 포함된 첫 요청은 그 전부를 단 한 번의 프롬프트에서 1.25× 신규 캐시 쓰기로 다시 지불하게 만듭니다. 프록시가 이를 방지합니다: 한 번도 이미지화하지 않은 세션은 이 일회성 비용을 손익분기 게이트에 반영하고, 여전히 이득일 때만 이미지로 전환합니다 — 그렇지 않으면 세션은 텍스트로 유지되고, 절감은 다음 새 세션부터 시작됩니다.

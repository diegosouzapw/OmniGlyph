...
服务商对**文本按 token 计费**,但对**图像按尺寸计费**——
与图像里塞了多少文字无关。一张标准页面无论文字多密集,成本都是固定的:
...
只有当精确计算表明确实划算,并且模型经过验证能够读出页面内容时,
OmniGlyph 才会进行这一转换。下面两套测试套件分别验证了这两个前提。
...
只有 ✅ 那一档会真正上线。任何读取效果不佳的方案都会**附带凭据**被阻止,
三态评分意味着猜错(`silent_wrong`)的模型会被视为比诚实弃权(`ILEGIVEL`)的模型更差。

...
# 🔌 Використання з клієнтами Claude
...
![Огляд: KPI-картки центру керування, спарклайн заощаджень і потік подій наживо](../../assets/dashboard-overview.png)
...
- **Бенчмарки** — підтвердження харнесу, відрендерені з `benchmarks/*/results/`, по одному рядку на експеримент модель·конфігурація, і **запуск бенчмарків прямо з UI**: dry-run за `$0` стрімлять свій вивід наживо; живі запуски залишаються за гейтом, що вимагає вашого API-ключа та явного підтвердження вартості.
- **Sessions / History** — топ сесій за заощадженими токенами та кожна подія на диску.

| Live Flow | Бенчмарки |
|---|---|
| ![Конвеєр запиту у вигляді живого графа вузлів](../../assets/dashboard-flow.png) | ![Підтвердження бенчмарків та dry-run прямо в UI](../../assets/dashboard-benchmarks.png) |

![Телеметрія: одометр і жива часова шкала запитів](../../assets/dashboard-telemetry.png)
...
# 📤 Офлайн-експорт — без проксі, без Claude Code
...
Ви отримуєте одну теку з усім необхідним, щоб перетягнути в чат:
...
**Я ввімкнув його посеред сесії, і витрати злетіли — чому?**

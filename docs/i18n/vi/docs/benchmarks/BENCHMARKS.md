...
## TL;DR — toàn bộ kết quả trong hai thanh

**Chi phí** — một trang tiêu chuẩn 1568×728 chứa 28,080 ký tự với chi phí
cố định 1,460 token; cùng văn bản đó gửi thô tốn ~10× nhiều hơn:
...
**Độ chính xác** — nhưng chỉ ở nơi mô hình thực sự đọc được trang. Cổng
chặn là fail-closed; chỉ hàng ✅ được đưa vào sản xuất:
...
Phần còn lại của tài liệu này là bằng chứng đứng sau hai thanh đó.
...
## 2. Độ chính xác đọc (density-frontier, needle hex/camelCase/chữ số + distractor)

### Ma trận 2×2 Fable 5 — qua CLI/đăng ký, n=30/nhánh, cùng corpus (~16.6k ký tự)
...
## 3. Chi phí theo từng nhà cung cấp (offline, chính xác — TRANG ĐẦY ĐỦ, lý thuyết)
...
## PHỤ LỤC 2026-07-06 (2) — Loạt kiểm tra GPT-5.5 qua API trực tiếp: H7 đã khép lại (THẤT BẠI)
...
## PHỤ LỤC 2026-07-06 (3) — Gemini 2.5-flash (MỘT PHẦN: hạn ngạch tier miễn phí cạn giữa chừng)

 ...
 * every request — OmniGlyph doesn't have a query layer, but a 1.5 MB JSONL
 ...
 *   GET  /api/stats.json             → full-history aggregate (formerly `OmniGlyph stats`)
 *
 ...
 *      state: OmniGlyph images the cached prefix in place (moves the caller's
...
interface SessionTotals {
  ...
  // Raw output tokens (the model's reply). OmniGlyph does NOT compress output, so
...
interface Totals {
   ...
   *  OmniGlyph or we can't measure what it would have cost, so the
   ...
   *  saved/actual exceed 100% because OmniGlyph shrunk the actual to
   *  near zero. */
 ...
 *  OmniGlyph actually paid — not a count_tokens probe. No per-session warmth state:
...
export class DashboardState {
  ...
  update(ev: ProxyEvent): void {
    ...
    if (gpt) {
      ...
      // OmniGlyph only reduces input by imaging the static slab. An UNCOMPRESSED
      ...
      // Cache-aware, server-observed baseline. INVARIANT: OmniGlyph is credited ONLY
    ...
    // denominator for "did OmniGlyph move my real bill". Measured rows
    // contribute their cache-aware baseline (what the unproxied path
    // would have billed); unmeasured/probe-failed/passthrough rows
    // contribute their actual input (OmniGlyph either didn't run or we
  ...
  serveStats(): Response {
    ...
    // answers "did OmniGlyph help on the rows where it ran" instead of
    // "did OmniGlyph move my real bill". The first is a cherry-pick.
    ...
    // paid with no OmniGlyph. Bounded ratio at 100%; a single cold-miss
    ...
    const payload = {
      ...
      // ask "is OmniGlyph helping". Negative when flap-pollution from
   ...
   *  runtime compress scope. In-memory only; restart resets to the OMNIGLYPH_MODELS

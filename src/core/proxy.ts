/**
 * OmniGlyph proxy as a single Web-standard fetch handler.
...
export interface ProxyConfig {
  ...
  /** When a TRANSFORMED request gets stop_reason:"refusal" (the classifier
   *  triggers on imaged context — stochastic per call, measured ~50% on
   *  transcription batches), re-sends the ORIGINAL body ONCE and serves that
   *  response. Fail-open. Default: true. */
  retryRefusalWithOriginal?: boolean;
...
function isCanonicalOpenAIPath(pathname: string, headers: Headers, hasOpenAIKey: boolean): boolean {
  ...
  // are a second reliable discriminator: never route those clients to OpenAI.
  const looksAnthropicClient =
    bearerIsAnthropic || headers.has('anthropic-version') || headers.has('anthropic-beta');
  const looksOpenAIAuth =
    !looksAnthropicClient && (hasOpenAIKey || (headers.has('authorization') && !headers.has('x-api-key')));
...
/** Refusal sniffer with lossless replay. For SSE, buffers only until the
 *  first sign of real content (content_block_delta) — normal responses pay
 *  ~first-token latency and then keep streaming; refusals end tiny (no
 *  deltas) and are detected at the end of the stream. For JSON, buffers the
 *  body (refusals have empty content). Fail-open: buffer cap exceeded or
 *  unknown format → verdict 'ok'. */
const REFUSAL_SNIFF_BUFFER_CAP = 256 * 1024;

async function sniffRefusal(res: Response): Promise<{ verdict: 'refusal' | 'ok'; replay: Response }> {
  const contentType = res.headers.get('content-type') ?? '';
  if (!res.ok || res.body === null) return { verdict: 'ok', replay: res };

  const replayOf = (body: BodyInit | ReadableStream<Uint8Array>): Response =>
    new Response(body as BodyInit, { status: res.status, statusText: res.statusText, headers: res.headers });
  ...
  if (contentType.includes('text/event-stream')) {
    ...
    let seen = '';
    let buffered = 0;
    let verdict: 'refusal' | 'ok' | undefined;
    let ended = false;
    while (verdict === undefined) {
      const { done, value } = await reader.read();
      if (done) {
        ended = true;
        verdict = /"stop_reason"\s*:\s*"refusal"/.test(seen) && !/"type"\s*:\s*"content_block_delta"/.test(seen)
          ? 'refusal'
          : 'ok';
        break;
      }
      ...
      if (/"type"\s*:\s*"content_block_delta"/.test(seen)) verdict = 'ok';
      else if (buffered > REFUSAL_SNIFF_BUFFER_CAP) verdict = 'ok'; // fail-open
    }
    ...
    return { verdict, replay: replayOf(replayBody) };
  }

  if (contentType.includes('application/json')) {
    const buf = new Uint8Array(await res.arrayBuffer());
    let verdict: 'refusal' | 'ok' = 'ok';
    try {
      const json = JSON.parse(new TextDecoder().decode(buf)) as {
        stop_reason?: unknown;
        content?: Array<{ type?: string; text?: string }>;
      };
      const hasText = Array.isArray(json.content)
        && json.content.some((b) => b?.type === 'text' && typeof b.text === 'string' && b.text.length > 0);
      if (json.stop_reason === 'refusal' && !hasText) verdict = 'refusal';
    } catch {
      // non-JSON body despite the content-type — fail-open
    }
    return { verdict, replay: replayOf(buf as BufferSource as BodyInit) };
  }

  return { verdict: 'ok', replay: res };
}
...
/** Parse OMNIGLYPH_GATEWAY_HEADERS — JSON object or `k=v;k2=v2`. */
...
export function createProxy(config: ProxyConfig = {}) {
  ...
  return async function handle(req: Request): Promise<Response> {
    ...
    let bodyOut: BodyInit | null;
    let info: TransformInfo | undefined;
    let requestModel: string | undefined;

    // Two count_tokens probes on the pre-compression body (see the HISTORY_CACHE_MODEL design note (omniglyph-history mirror)):
    ...
    /** Original pre-transform body — fuel for the retry-on-refusal. */
    ...
    if (isMessages || isOpenAIChat || isOpenAIResponses) {
      ...
      try {
        ...
        return new Response(JSON.stringify({ error: 'OmniGlyph transform failed' }), {
          status: 502,
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'omniglyph upstream unreachable' }), {
        status: 502,
        headers: { 'content-type': 'application/json' },
      });
    }

    // Retry-on-refusal (upstream #37): a classifier refusal on a TRANSFORMED
    // request is stochastic per call — the same content without images
    // usually passes. Re-sends the original body ONCE; fail-open on any
    // retry failure (the original refusal is delivered).
    if (
      isMessages &&
      info?.compressed === true &&
      originalBody !== undefined &&
      config.retryRefusalWithOriginal !== false &&
      upstreamRes.ok
    ) {
      const sniff = await sniffRefusal(upstreamRes);
      if (sniff.verdict === 'refusal') {
        try {
          const retryRes = await fetch(upstreamUrl, {
            method: req.method,
            headers: outHeaders,
            body: originalBody as BufferSource as BodyInit,
          });
          info.refusalRetried = true;
          upstreamRes = retryRes;
        } catch {
          upstreamRes = sniff.replay; // retry unreachable → serve the original refusal
        }
      } else {
        upstreamRes = sniff.replay;
      }
    }

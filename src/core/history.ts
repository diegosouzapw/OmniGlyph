...
import { DENSE_CONTENT_CHARS_PER_IMAGE, DENSE_CONTENT_COLS, DENSE_RENDER_STYLE, neutralizeSentinel, reflow, roleSlotSegment, SLOT_MARK_ASSISTANT, SLOT_MARK_USER } from './render.js';
...
export interface HistoryCollapseOptions {
  ...
  /** Dense-page render columns. Injected per vision tier by transform.ts
   *  (384 on high-res models, 312 standard). Default: DENSE_CONTENT_COLS. */
  denseCols: number;
  /** Char budget per rendered page; must equal denseCols × page rows so image
   *  estimates track real pagination. Default: DENSE_CONTENT_CHARS_PER_IMAGE. */
  denseCharsPerImage: number;
  /** Page-height ceiling in px (728 standard, 1928 high-res). `undefined`
   *  falls through to the renderer's standard default. */
...
export const HISTORY_DEFAULTS: HistoryCollapseOptions = {
  ...
  denseCols: DENSE_CONTENT_COLS,
  denseCharsPerImage: DENSE_CONTENT_CHARS_PER_IMAGE,
 ...
 * demoteProtectedHeadText, so OmniGlyph scaffolding is never mistaken for the task.
 */
...
function demoteProtectedHeadText(
  ...
  return head.map((m, idx) => {
    ...
    // OmniGlyph's own slab scaffolding (the rendered images, the fact-sheet, and the
...
export async function collapseHistory(
  ...
  // across turns and keeps hitting Anthropic's prompt cache. See the HISTORY_CACHE_MODEL design note (omniglyph-history mirror).
  ...
  for (const chunkEnd of sortedEnds) {
    ...
    const imgs = await renderTextToPngsCached(
      chunkRender,
      o.denseCols,
      o.denseCharsPerImage,

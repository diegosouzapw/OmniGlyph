 ...
 *  1-BIT atlas (aa:false), measured 2026-07-05 on Fable 5 (density-frontier,
 *  n=30/arm): 1-bit 30/30 vs AA 25/30 on the standard page and 20/30 vs 0/30
 *  on high-res, zero confabulation across the 4 arms. The previous AA had been
 *  calibrated on Opus 4.7 — which cannot read the 5×8 anyway (30/30 ILLEGIBLE). */
export const DENSE_RENDER_STYLE: RenderStyle = { cellWBonus: 0, cellHBonus: 0, aa: false };
...
export interface RenderedImage {
  ...
  /** Escape-exempt codepoints absent from atlas (invisibles/modifiers — see
   *  isEscapeExempt), rendered as blank cells. Non-exempt misses are escaped to
   *  `[U+HEX]` before wrap and never count here. Surface as telemetry. */
  droppedChars: number;
 ...
 *  OmniGlyph itself (rendered dumps, OCR, this very transcript). {@link neutralizeSentinel}
...
export function measureContentCols(text: string, maxCols: number, markerScale: number = 1): number {
  ...
  for (let i = 0; i <= text.length; i++) {
    if (i === text.length || text[i] === '\n') {
      const w = measureLineCols(escapeMissingGlyphs(expandTabsInLine(text.slice(start, i))), markerScale);
...
export async function renderChunkToPng(
  ...
  let png: Uint8Array;
  if (colorMask) {
    // colorCycle / colorByRole / colorByClass: AA-blend ink onto paper in palette color. markerRed ignored.
...
export interface RenderDensePagesOptions {
   ...
   *  that pre-reflow (the proxy's maybeReflow / history lockstep) pass false; `omniglyph export`
 ...
 * `renderTextToImages` (library.ts → `omniglyph export`) AND the proxy's `textToImageBlocks`

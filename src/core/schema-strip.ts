...
/** Metadata keys that add tokens but no validation; the image carries them for
 *  the model. `$schema`/`$id` are NOT included here (upstream #45): removing
 *  them changes validation SEMANTICS — without `$schema`, the API validates as
 *  2020-12 and rejects draft-07 constructs (e.g. tuple-form `items`) with a
 *  400 in a silent loop. */
...
/** JSON Schema keys whose value is a single subschema — ou, no caso de
 *  `items` (draft-07) e `prefixItems` (2020-12), possivelmente uma TUPLA de
 *  subschemas, stripada elemento a elemento. */
...
const SCHEMA_VERBATIM_KEYS = new Set([
  ...
  '$schema',       // dialect pin (draft-07 vs 2020-12) — validation semantics
  '$id',           // base URI for $ref resolution
  'minimum',
  'maximum',
  'exclusiveMinimum',
  'exclusiveMaximum',
...
export function stripSchemaDescriptions(node: unknown, depth = 0): unknown {
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (SCHEMA_SINGLE_SUBSCHEMA_KEYS.has(k)) {
      // additionalProperties/additionalItems may be a boolean — pass through untouched.
      if (typeof v === 'boolean') {
        out[k] = v;
      } else if (Array.isArray(v)) {
        // TUPLE-form items (draft-07) / prefixItems: preserve the array
        // (collapsing it changes the contract) and strip each element.
        out[k] = v.map((sub) => stripSchemaDescriptions(sub, depth + 1));
      } else {

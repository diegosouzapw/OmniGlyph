...
export interface OmniGlyphOptions
  extends Pick<
    TransformOptions,
...
export interface OmniGlyphTransformInput {
  ...
  readonly options?: OmniGlyphOptions;
}

export type OmniGlyphReason =
  | 'applied'
  | 'unsupported_model'
  | 'parse_error'
  | 'below_min_chars'
...
export interface OmniGlyphTransformResult {
  readonly body: Uint8Array;
  readonly applied: boolean;
  readonly reason: OmniGlyphReason;
...
function classifyReason(info: TransformInfo): OmniGlyphReason {
...
export interface RenderTextToImagesOptions {
   ...
   *  what `omniglyph export` uses. Default false (raw one-line-per-row). */

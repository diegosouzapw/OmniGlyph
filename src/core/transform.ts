...
export interface TransformOptions {
  ...
  /** Image the static system-prompt + tool-docs slab. `false` keeps ALL
   *  session config as native text — no slab render, no system-field
   *  rewrite, no env relocation, no tool-doc stubbing (stubbed docs ride
   *  inside the imaged slab, so without it they would be lost), and
   *  first-message `<system-reminder>` blocks stay text — while tool_result
   *  and history compression still run. Guard for Anthropic's
   *  reasoning_extraction refusal classifier, which fires on
   ...
   *  Default 1 (per-turn cold gate). See the HISTORY_CACHE_MODEL design note (omniglyph-history mirror). */
...
const DEFAULTS: Required<TransformOptions> = {
  ...
  // 312 cols × 5 px + 8 px pad = 1568 px — exactly the standard-tier long-edge
  // bound (313 cols = 1573 px triggers a 0.997× resample there and bills an
  // extra 28 px patch column on every tier). Per-tier geometry may override
  // this with pageGeometryForTier() unless the caller set cols explicitly.
  cols: 312,
...
//
// Image token cost is EXACT: 28 px patches per tier-resized image plus a fixed
// per-block overhead (anthropic-vision.ts; residual 0 vs the 2026-07-05
// count_tokens sweep on both tiers). Text-side constants still bias
// CONSERVATIVE: CHARS_PER_TOKEN=4 under-estimates text savings, so
// mispredictions leave money on the table; they never generate net-loss images.
...
/** Chars-per-token for the `omniglyph export` *reporting* estimate (factsheet & savings %).
...
/** RETIRED billing formula (`tokens ≈ w×h/750`), kept ONLY for the offline
 *  export/report pipeline (export.ts). The live gate now uses the exact
 *  28 px-patch model in anthropic-vision.ts, measured to residual 0 on the
 *  2026-07-05 count_tokens sweep — /750 over-prices a 1568×728 page by ~4.5%. */
export const ANTHROPIC_PIXELS_PER_TOKEN = 750;
/** Report-only companion to ANTHROPIC_PIXELS_PER_TOKEN (export.ts). The live
 *  gate no longer applies a margin: patch math is exact, and the per-block
 *  overhead constant (+4 vs measured +3) already leans conservative. */
...
function imageTokensForRows(
  ...
  tier: AnthropicVisionTier = 'standard',
): number {
  if (!Number.isFinite(visualRows) || visualRows <= 0) return 0;
  const n = Math.max(1, numCols | 0);
  const widthPx = multiColWidthPx(cols, n);
  const pageHeightPx = pageGeometryForTier(tier).maxHeightPx;
  const hardLinesPerImg = Math.max(1, Math.floor((pageHeightPx - 2 * PAD_Y) / CELL_H));
  ...
  // Exact per-image billing: tier-resized 28 px patches + fixed block overhead.
  const perFull = anthropicImageTokens(widthPx, fullImageHeight, tier) + ANTHROPIC_IMAGE_BLOCK_OVERHEAD_TOKENS;
  const perLast = anthropicImageTokens(widthPx, lastImageHeight, tier) + ANTHROPIC_IMAGE_BLOCK_OVERHEAD_TOKENS;
  return fullImages * perFull + perLast;
}
...
function imageTokensCost(
  ...
  return imageTokensForRows(rows, effectiveCols, numCols, imageCountCap, maxCharsPerImage, tier);
}
...
function maybeReflow(text: string, enabled: boolean): string {
  ...
  // content is about OmniGlyph itself). Render-only; originals are preserved via
...
export function evalCompressionProfitability(
  ...
  const cpt = Number.isFinite(charsPerToken) && charsPerToken > 0
    ? charsPerToken
    : CHARS_PER_TOKEN;
  const imageTokens = imageTokensCost(text, cols, n, imageCountCap, shrinkWidth, maxCharsPerImage, tier);
...
export function isCompressionProfitable(
  ...
  const cpt = Number.isFinite(charsPerToken) && charsPerToken > 0
    ? charsPerToken
    : CHARS_PER_TOKEN;
  const imageTokensCost_ = imageTokensCost(text, cols, n, imageCountCap, shrinkWidth, maxCharsPerImage, tier);
 ...
 * Falls back to cold per-turn gate when `horizon <= 1`. See the HISTORY_CACHE_MODEL design note (omniglyph-history mirror).
 */
export function isCompressionProfitableAmortized(
  ...
  if (!Number.isFinite(horizon) || horizon <= 1) {
    return isCompressionProfitable(text, cols, imageCountCap, numCols, charsPerToken, priorWarmTokens, priorWarmImageTokens, shrinkWidth, maxCharsPerImage, tier);
  }
...
export interface TransformInfo {
  compressed: boolean;
  reason?: string;
  /** Set by the PROXY (not by the transform): the transformed request got
   *  stop_reason:"refusal" from the classifier and was re-sent with the
   *  ORIGINAL body — this response is the retry's. Telemetry for failure mode #37. */
  ...
  imageTokens?: number;
  /** GPT only. o200k_base text tokens of the content OmniGlyph imaged/stripped —
   ...
   *  ⇒ OmniGlyph serialized different prefix bytes (we busted our own cache,
   *  OmniGlyph-side); STABLE while cache_create spikes / cache_read collapses ⇒ the
...
/**
 * After a history collapse, move OmniGlyph's single relocated cache breakpoint off
 ...
 * the total marker count never increases (OmniGlyph never *adds* — only moves).
 */
...
/**
 * Read-only digest of the cacheable prefix OmniGlyph actually sends: tools +
 ...
 * serialized different prefix bytes (OmniGlyph-side — a per-turn block crossing the
...
async function cachePrefixDigest(
  ...
  // Boundary = latest message carrying OmniGlyph's imaged prefix: the history image
  ...
  for (const m of msgs) {
    ...
    if (Array.isArray(m.content)) {
      for (const block of m.content) {
        const probe = block as { type?: unknown; text?: unknown } | null;
        if (probe && probe.type === 'text' && typeof probe.text === 'string') {
          return probe.text.slice(0, 4096);
        }
      }
    }
...
function buildPagingMarker(args: {
  ...
  return (
    `\n\n[ OmniGlyph paging: omitted ${args.omittedLines.toLocaleString('en-US')} lines ` +
...
export function truncateForBudget(
  ...
  const n = Math.max(1, numCols | 0);
  const estImages = estimateImageCount(text, cols, n, maxCharsPerImage, maxHeightPx);
  ...
  const readableLinesPerCol = Math.max(1, Math.floor(maxCharsPerImage / Math.max(1, cols)));
  const totalRowBudget = Math.max(8, maxImages * Math.min(hardLinesPerImg, readableLinesPerCol) * n - 6);
 ...
 * `omniglyph export` CLI emit byte-identical PNGs for the same text. Exported so
...
export async function textToImageBlocks(
  ...
  shrinkWidth: boolean = true,
  /** Dense-page geometry (per vision tier). Default = standard 1568×728 page. */
  page: PageGeometry = STANDARD_PAGE,
): Promise<{
  blocks: ImageBlock[];
  ...
  const imgs =
    effectiveNumCols > 1
      ...
      // Single-col dense: shrink the page-cols base to content so the renderer matches
      // the gate (denseGateGeometry uses page.cols, priced via shrinkColsToContent).
      // Was hard-coded to DENSE_CONTENT_COLS, which threw away the shrink the gate assumed.
      : await renderTextToPngsCached(renderText, shrinkColsToContent(renderText, page.cols), page.charsPerImage, DENSE_RENDER_STYLE, page.maxHeightPx);
  let droppedChars = 0;
  let pixels = 0;
...
async function runHistoryCollapseAndFinalize(
  ...
  if (Array.isArray(req.messages) && req.messages.length > 0) {
    ...
    const historyProfitable = (text: string, cols: number): boolean => {
      // History always renders single-col at the tier's dense page geometry
      // (history.ts → renderTextToPngsWithCharLimit with the injected page
      // cols/chars), so gate at THAT geometry, not o.cols.
      const g = denseGateGeometry(cols, 1, page);
      return isCompressionProfitableAmortized(
        text, g.cols, undefined, 1, historyCpt, horizon,
        o.priorWarmTokens, o.priorWarmImageTokens, true, g.maxChars, tier,
      );
    };
...
export async function transformRequest(
  ...
  // Per-tier page geometry (measured 2026-07-05): high-res models (Fable 5,
  // Opus 4.8, Sonnet 5, …) read 1928×1928 pages WYSIWYG, so they get the big
  // 384×240 page; everything else keeps the audited 1568×728. An explicit
  // caller `cols` still wins for the slab width.
  const tier = resolveAnthropicVisionTier(req.model);
  const page = pageGeometryForTier(tier);
  const slabColsBase = opts.cols !== undefined ? o.cols : page.cols;
  ...
  // stays native text — skip the slab gate/render, the system-field rewrite +
  // first-message splice (sections 4/5a), AND tool-doc compression (section 2:
  // stubbed docs ride inside the imaged slab, so without it they would be
  // silently destroyed); tool_result (5b) and history (6) compression still
  // run. Section 6's protectedPrefix shields the opening config-bearing user
  // message, and demoteProtectedHeadText keeps its <system-reminder> blocks
  // verbatim. Guard for Anthropic's reasoning_extraction refusal classifier,
  // which fires on system-prompt-shaped content rendered inside user-message
  // images (2.6% refusal rate on reminder-imaged requests vs 0% uncompressed,
  // events.jsonl 2026-07-11).
  ...
  if (o.compressTools && !keepSystemText && Array.isArray(req.tools) && req.tools.length > 0) {
    const docs: string[] = [];
    ...
    toolsRewritten = req.tools.map((t) => {
      ...
      // client's silent fallback switches models, zeroing the session's
      // savings (upstream #44). They pass through untouched and stay out of
      // the imaged Tool Reference.
      const toolType = (t as { type?: unknown }).type;
      if (typeof toolType === 'string' && toolType !== 'custom') return t;
  ...
  // that defused the imaged-slab banner refusal (169521c): OmniGlyph names itself
  ...
  const toolReferenceText = toolDocsText
    ? '=== TOOL REFERENCE ===\n' +
      "OmniGlyph (this user's local proxy) moved the full tool documentation for this" +
  ...
  const denseGeo = denseGateGeometry(o.cols, numCols, page);
  ...
  if (!keepSystemText) { // keepSystemText: slab stays text (see the note above section 2)
    ...
    const slabCols = shrinkColsToContent(combinedWithHeader, slabColsBase);
    ...
    const images =
      numCols > 1
        ...
        : await renderTextToPngsCached(combinedWithHeader, slabCols, page.charsPerImage, {}, page.maxHeightPx);
  ...
  // Images go into first user message — system field rejects images (400 system.N.type).
  {
    if (!keepSystemText) {
      ...
      if (firstUserIdx >= 0) {
        ...
        //     moves to the LAST produced image (OmniGlyph never adds its own markers).
        ...
        if (o.compressReminders) {
          for (const blk of existing) {
            ...
            if (!isCompressionProfitable(reminderText, denseGeo.cols, undefined, numCols, o.charsPerToken, 0, 0, true, denseGeo.maxChars, tier)) {
              bumpPassthrough(info, 'not_profitable');
              processedExisting.push(blk);
              continue;
            }
          ...
          if (blk && (blk as ToolResultBlock).type === 'tool_result') {
            ...
            if (typeof innerRaw === 'string') {
              ...
              if (innerR.length < o.minToolResultChars) {
                bumpPassthrough(info, 'below_threshold');
                rewritten.push(blk);
              } else if (!isCompressionProfitable(innerR, denseGeo.cols, o.maxImagesPerToolResult, numCols, o.charsPerToken, 0, 0, true, denseGeo.maxChars, tier)) {
                bumpPassthrough(info, 'not_profitable');
                rewritten.push(blk);
              } else {
                // Paging: truncate before render if it would blow the image cap.
                const paged = truncateForBudget(innerR, o.maxImagesPerToolResult, denseGeo.cols, numCols, denseGeo.maxChars, page.maxHeightPx);
                if (paged.truncated) {
              ...
              for (const ib of innerRaw) {
                ...
                if (!isCompressionProfitable(innerTextR, denseGeo.cols, o.maxImagesPerToolResult, numCols, o.charsPerToken, 0, 0, true, denseGeo.maxChars, tier)) {
                  bumpPassthrough(info, 'not_profitable');
                  newInner.push(ib as TextBlock | ImageBlock);
                  continue;
                }
                const paged = truncateForBudget(innerTextR, o.maxImagesPerToolResult, denseGeo.cols, numCols, denseGeo.maxChars, page.maxHeightPx);
                if (paged.truncated) {
                  info.truncatedToolResults = (info.truncatedToolResults ?? 0) + 1;
                  info.omittedChars = (info.omittedChars ?? 0) + paged.omittedChars;
                }
  ...
  if (Array.isArray(req.messages) && req.messages.length > 0) {
    ...
    const horizon = Math.max(1, Math.floor(o.historyAmortizationHorizon));
    const historyProfitable = (text: string, cols: number): boolean => {
      // Gate at the tier's dense page geometry (matches history.ts renderer).
      const g = denseGateGeometry(cols, 1, page);
      return isCompressionProfitableAmortized(
        text, g.cols, undefined, 1, historyCpt, horizon,
        o.priorWarmTokens, o.priorWarmImageTokens, true, g.maxChars, tier,
  ...
  // so this block is never self-imaged; and OmniGlyph is stateless per request,
  // so the wrapper never appears in inbound client history (no compounding).
  if (volatileEnvText) {
    const wrappedEnvText = `<system-reminder>\nContext relocated by OmniGlyph from the system prompt (volatile per-turn environment state — not written by the user):\n\n${volatileEnvText}\n</system-reminder>`;
    const msgs = req.messages ?? [];

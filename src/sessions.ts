 ...
 * better-than-good-enough session key without coupling OmniGlyph to Claude
 ...
 *
 * - `~/.omniglyph/events.jsonl` — append-only JSONL written by FileTracker
 * - `~/.omniglyph/4xx-bodies/${iso-ts}-${sha8}.json.gz` — gzipped failure
...
export function defaultPaths(): SessionsPaths {
  ...
  const eventsFile =
    process.env.OMNIGLYPH_LOG ?? path.join(home, '.omniglyph', 'events.jsonl');
 ...
 *  lines are silently dropped — matches `OmniGlyph stats` behavior. */
...
export async function aggregateSessions(
  ...
  for await (const { ev, rawBytes } of readEvents(paths.eventsFile)) {
    ...
    // full derivation: OmniGlyph is credited only for token reduction, never for
 ...
 * missing, returns an empty map without throwing — OmniGlyph must keep

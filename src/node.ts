...
function applyConfigFileDefaults(): void {
  const file = process.env.OMNIGLYPH_CONFIG ?? DEFAULT_CONFIG_FILE;
  if (!fs.existsSync(file)) return;
  let parsed: unknown;
  try {
    parsed = JSON.parse(fs.readFileSync(file, 'utf8')) as unknown;
  } catch (e) {
    console.warn(`[OmniGlyph] ignored invalid config ${file}: ${(e as Error).message}`);
    return;
  }
...
function parseCli(argv: string[]): RuntimeConfig {
  ...
  for (const a of argv) {
    ...
    if (a.startsWith('-')) {
      console.error(`[OmniGlyph] unknown option: ${a}`);
      console.error(`[OmniGlyph] this build accepts no flags; run \`OmniGlyph --help\` for env vars`);
      process.exit(2);
    }
  }
  applyConfigFileDefaults();
  const sharedUpstream = process.env.OMNIGLYPH_UPSTREAM;
  return {
    ...
    eventsFile:
      process.env.OMNIGLYPH_LOG ??
      path.join(os.homedir(), '.omniglyph', 'events.jsonl'),
    // Off by default: 4xx request bodies hold full prompts + any secrets in
    // context. Opt in for debugging only.
    captureErrorReqBody: process.env.OMNIGLYPH_DEBUG_CAPTURE_4XX === '1',
  };
}
...
function parseProvider(v: string | undefined): 'cloudflare-ai-gateway' | undefined {
  ...
  console.error(`[OmniGlyph] unknown OMNIGLYPH_PROVIDER: ${v}`);
  process.exit(2);
}

function printHelp(): void {
  console.log(t('cli.help', resolveLocale(process.env)));
}
...
// inside its own run-script env, so for `npx omniglyph` or a global bin it is
// undefined (or reflects the *consumer's* package), never this tool's version.
declare const __OMNIGLYPH_VERSION__: string | undefined;

function printVersion(): void {
  const injected = typeof __OMNIGLYPH_VERSION__ === 'string' ? __OMNIGLYPH_VERSION__ : undefined;
...
class FileTracker implements Tracker {
  ...
  private ensureOpen(): boolean {
    ...
    try {
      ...
      if (!this.brokenLogged) {
        console.error(
          `[OmniGlyph] FileTracker disabled — cannot open ${this.filePath}: ${(err as Error).message}`,
        );
  ...
  emit(ev: TrackEvent): void {
    ...
    try {
      ...
      if (!this.brokenLogged) {
        console.error(
          `[OmniGlyph] FileTracker write failed: ${(err as Error).message}`,
        );
        this.brokenLogged = true;
      }
    }
...
function printExportHelp(): void {
  console.log(t('cli.exportHelp', resolveLocale(process.env)));
}
...
function collectFilesFromTargets(
  ...
  for (const target of targets) {
    ...
    try { st = fs.statSync(target); } catch {
      console.warn(`[omniglyph export] skipping inaccessible target: ${target}`);
      continue;
    }
    if (st.isDirectory()) {
      ...
      if (r.kind === 'ok') files.push({ relPath: rel, content: r.content });
        ...
        console.warn(`[omniglyph export] skipping ${r.kind} file: ${target}`);
      }
    }
  }
  return files;
...
async function collectSource(opts: ExportParsed): Promise<[string, string[]]> {
  ...
  if (opts.git) {
    ...
    for (const rel of untrackedFiles) {
      ...
      if (r.kind === 'ok') untracked += `\n===== ${rel} =====\n` + r.content;
        ...
        console.warn(`[omniglyph export] skipping ${r.kind} untracked file: ${rel}`);
      }
    }
  ...
  if (files.length === 0) {
    console.warn('[omniglyph export] no files collected');
  }
...
async function runExport(argv: string[]): Promise<void> {
  ...
  // Unique output dir: <out>/OmniGlyph-export-XXXXXX/. mkdtemp guarantees a fresh, random
  // directory so concurrent runs never collide and stale page-NNN.png never bleed in.
  fs.mkdirSync(opts.out, { recursive: true });
  const outDir = fs.mkdtempSync(path.join(opts.out, 'OmniGlyph-export-'));
...
async function main(): Promise<void> {
  ...
  // No subcommands — OmniGlyph is just the proxy. Stats / sessions / cleanup
  // tools live in the dashboard (see http://127.0.0.1:${port}/).
  const opts = parseCli(argv);
  // A/B harness passthrough switch (see the `transform` callback below).
  const forcePassthrough = /^(1|true|yes|on)$/i.test(process.env.OMNIGLYPH_DISABLE ?? '');
  if (forcePassthrough) {
    console.log('[OmniGlyph] OMNIGLYPH_DISABLE set — passthrough mode (compress=false), still logging usage + baselines');
  }
  // Debug aid: when OMNIGLYPH_DUMP_DIR is set, persist every rendered PNG this
  ...
  // Note: the OMNIGLYPH_DISABLE arm renders nothing, so only the compress proxy
  // produces files here.
  let imageDumpDir: string | undefined = process.env.OMNIGLYPH_DUMP_DIR?.trim() || undefined;
  let imageDumpSeq = 0;
  if (imageDumpDir) {
    try {
      fs.mkdirSync(imageDumpDir, { recursive: true });
      console.log(`[OmniGlyph] OMNIGLYPH_DUMP_DIR set — dumping rendered PNGs to ${imageDumpDir}`);
    } catch (err) {
      console.warn(`[OmniGlyph] OMNIGLYPH_DUMP_DIR unusable (${(err as Error).message}) — image dumping disabled`);
  ...
  const config: ProxyConfig = {
    ...
    onRequest: async (e) => {
      ...
      dashboard.update(e);
      // Debug: persist this request's rendered PNGs (see OMNIGLYPH_DUMP_DIR above).
      ...
      if (imageDumpDir && e.info?.imagePngs && e.info.imagePngs.length > 0) {
        ...
        for (let i = 0; i < pngs.length; i++) {
          ...
          try {
            ...
            console.warn(`[OmniGlyph] PNG dump write failed: ${(err as Error).message}`);
      ...
      if (e.errorBody) {
        const trimmed = e.errorBody.length > 400
          ...
          : e.errorBody;
        console.warn(`[OmniGlyph ${e.status}] upstream body: ${trimmed}`);
      }
      ...
      if (e.info?.unknownStaticTags && e.info.unknownStaticTags.length > 0) {
        console.warn(
          `[OmniGlyph warn] unknown tag(s) in static slab: ${e.info.unknownStaticTags.join(', ')}  ` +
      ...
      // Persistent JSONL event for offline analysis (OmniGlyph stats etc.).
  ...
  const server = createServer((req, res) => {
    Promise.resolve()
      ...
      .catch((err) => {
        console.error('[OmniGlyph] handler error:', err);
  ...
  server.listen(opts.port, opts.host, () => {
    console.log(`[OmniGlyph] listening on http://${displayHost}:${opts.port}`);
    if (!isLoopbackHost) {
      console.warn(
        `[OmniGlyph] WARNING: bound to ${opts.host} — the unauthenticated dashboard ` +
    ...
    console.log(`[OmniGlyph] anthropic upstream → ${routes.anthropic}`);
  ...
  const shutdown = (sig: string) => {
    if (shuttingDown) {
      console.log(`[OmniGlyph] ${sig} again — forcing exit`);
      process.exit(130);
    }
    shuttingDown = true;
    console.log(`[OmniGlyph] ${sig} — shutting down`);

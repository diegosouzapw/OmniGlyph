...
export interface Env {
  ...
  /** Semantic transform boundary; aggressive preserves the legacy default. */
   ...
   *    npx wrangler secret put OMNIGLYPH_WORKER_SECRET */
  OMNIGLYPH_WORKER_SECRET?: string;
}
...
export default {
  async fetch(req: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    ...
    if (env.ANTHROPIC_API_KEY || env.OPENAI_API_KEY) {
      if (!env.OMNIGLYPH_WORKER_SECRET) {
        return new Response(
          JSON.stringify({
            error:
              'refusing to proxy: an API key override is configured but OMNIGLYPH_WORKER_SECRET is not, ' +
              'which would let anyone who finds this URL spend the configured key. ' +
              'Run `npx wrangler secret put OMNIGLYPH_WORKER_SECRET` and send the value as the x-omniglyph-secret header.',
          }),
          { status: 503, headers: { 'content-type': 'application/json' } },
        );
      }
      const presented = req.headers.get('x-omniglyph-secret') ?? '';
      if (!(await secretsMatch(presented, env.OMNIGLYPH_WORKER_SECRET))) {
        return new Response(
          JSON.stringify({ error: 'missing or invalid x-omniglyph-secret header' }),
    ...
    const sharedUpstream = env.OMNIGLYPH_UPSTREAM;

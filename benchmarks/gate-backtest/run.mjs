...
// from the proxy log. Answers "when does OmniGlyph actually save vs cost tokens,
// and could a smarter gate avoid the losses?" — re-run this per model release
// (the reconciliation node.ts asks for before wiring a live-feedback gate).
//
// Streams the log line-by-line, so it works on a multi-GB events.jsonl without
// slurping the whole file into memory.
//
// Scoring is faithful and mirrors src/core/baseline.ts exactly:
//   actual_eff   = input + cc·1.25 + cr·0.1            (what the imaged request paid)
...
//
//   node benchmarks/gate-backtest/run.mjs                 # ~/.omniglyph/events.jsonl (or $OMNIGLYPH_LOG)
//   node benchmarks/gate-backtest/run.mjs <log> [family]  # e.g. ... events.jsonl fable
//
...
if (!fs.existsSync(logFile)) {
  console.error(`[gate-backtest] no log at ${logFile} (pass a path or set OMNIGLYPH_LOG)`);
  process.exit(1);
}
